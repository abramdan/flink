...
//    val testingDS = env.readCsvFile[(Int, Int)](pathToData)
    val test = expectedResult.map(x => (x._1, x._2))
    val testingDS = env.fromCollection(test)

//    testingDS.writeAsCsv("/home/ghermann/tmp/teszt001.csv",
    ...
    val pred = predictedRatings.collect()
    //    predictedRatings.writeAsCsv("/home/ghermann/tmp/sgd_001.csv",
    // writeMode = FileSystem.WriteMode.OVERWRITE).setParallelism(1)
